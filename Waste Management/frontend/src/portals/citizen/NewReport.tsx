...
export default function NewReport() {
  ...
  // Drastically speeds up Vision API by compressing 10MB camera photos to ~100KB
  ...
  async function onPhoto(selected: File) {
    ...
    try {
      // Compress the image before uploading to save bandwidth and backend decode time
      const compressedBlob = await compressImage(selected);
      
      const form = new FormData();
      form.append('file', compressedBlob, selected.name);
